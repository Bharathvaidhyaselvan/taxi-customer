...
import { screenHeight, screenWidth, trip_details, search_loader, normal, promo_codes, bold, GOOGLE_KEY, month_names, money_icon, discount_icon, no_favourites, add_favourite, get_home, api_url, img_url, get_estimation_fare, pin_marker, regular, get_zone, btn_loader, ride_confirm, trip_request_cancel, f_m, search_new } from '../config/Constants';
...
const Dashboard = (props) => {
  ...
  const booking_sync = () => {
    database().ref(`customers/${global.id}`).on('value', snapshot => {
      ...
      //console.log(snapshot.val().is_searching)
  ...
  const load_trip_types = () => {
    ...
    return trip_types.map((data) => {
      ...
      return (
        <TouchableOpacity activeOpacity={1} onPress={change_trip_type.bind(this, data)} style={{ alignItems: 'center', justifyContent: 'center', width: '30%' }}>
      ...
      return (
        <DropShadow
          style={{
            ...
            marginBottom: 5,
            marginTop: 5,
            shadowColor: colors.theme_fg_two,
            shadowOffset: {
              width: 0,
              height: 0,
            },
            shadowOpacity: active_vehicle_type == data.id ? 0.5 : 0,
            shadowRadius: 5,
          }}
        >
  ...
  const get_estimation_fare_api = async (lat1, lng1, lat2, lng2, package_id, sub_type, pr) => {
    console.log({ customer_id: global.id, pickup_lat: lat1, pickup_lng: lng1, drop_lat: lat2, drop_lng: lng2, trip_type: active_trip_type, promo: pr, lang: global.lang, package_id: package_id, days: 1, trip_sub_type: sub_type })
    setLoading(true);
    await axios({
      method: "post",
      ...
      .catch((error) => {
        setLoading(false);
        alert("Sorry something went wrong4");
      });
  };
  ...
  const call_zone = async () => {
    ...
    await axios({
      ...
      .then(async (response) => {
        if (response.data.result == 0 || response.data.result == null) {
          setLoading(false);
      ...
      .catch((error) => {
        setLoading(false);
        alert("Sorry something went wrong5");
      });
  }

  const call_ride_confirm = async () => {
    console.log({
      km: km,
      promo: promo,
    ...
    await axios({
      ...
      .catch((error) => {
        ...
        console.log(error)
        setLoading(false);
        alert("Sorry something went wrong6"+-+error);
      });
  }
      ...
      <View>
        ...
        <Animated.View style={[{ transform: [{ translateY: book_comp_1 }] }, [{ position: 'absolute', width: '100%', height: (screenHeight - 250), paddingBottom: 10, justifyContent: 'flex-start', backgroundColor: colors.theme_bg_three }]]}>
          ...
          <View style={{ height: 140, alignItems: 'center', justifyContent: 'flex-end' }}>
            <View style={{flexDirection:'row', width: '100%', height: 30, backgroundColor: colors.orange, alignItems: 'center', justifyContent: 'center' }}>
            <Icon type={Icons.Ionicons} name="warning" color={colors.theme_fg_three} style={{ fontSize: 20 }} />
            <View style={{ margin: 5 }} />
              <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: 12, fontFamily: normal, letterSpacing: 1 }}>Toll fare and Parking costs are etc.,</Text>
            </View>
